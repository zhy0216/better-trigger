...
| [06-published-package-metadata.md](./done/06-published-package-metadata.md) | P2 | easy | ✅ 已完成 · core/db/kernel 补 `engines.node >=18`；入口副作用审计后按包声明 `sideEffects`（core/db/kernel `false`，SDK 因 registry 写 globalThis 声明整个 dist bundle 图，worker 仅 `main.*`/`embedded.*`），并由 `check:pkg-meta` 对五个 tgz manifest + bundler tree-shaking fixture 把关 |
| [07-ci-delivery-gates.md](./done/07-ci-delivery-gates.md) | P1 | hard | ✅ 已完成 · CI 安装后阻断 high audit，以双 provenance/artifact guard 验证当前 SHA、旧 SHA 与 orphan，复用集中脚本覆盖五包 Node 18/20/22 metadata smoke，并以隔离 PostgreSQL runtime smoke 验证非 root、migration、只读 tasks、deep health 与 SIGTERM 清理 |
...
6. `06-published-package-metadata.md`（P2，easy）✅ 已完成，归档至 `done/` — 依赖 01、02；新增根侧 `bun run check:pkg-meta`（pack manifest + tree-shaking fixture + Node 入口 smoke），07 直接调用它即可。
7. `07-ci-delivery-gates.md`（P1，hard）✅ 已完成，归档至 `done/` — 依赖 01、02、04、06；统一修改 `.github/workflows/ci.yml`。
