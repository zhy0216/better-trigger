...
- `POST /api/v1/runs/:id/retry`:仅 failed/canceled;**创建新 run**(同 payload,trigger_type='retry',attempt=1,无缓存 steps),返回 `{runId}`。
  - **请求级幂等(可选,`Idempotency-Key` 请求头)**:带上该头时,这次 retry 操作按 `(project_id, env, 源 run id, key)` 记入独立的 `run_retry_operations` 表(主键即该四元组);**同一客户端对同一次意图**的重复发送——双击落在同一意图内的第二次点击、请求未 settle 前的重发、反向代理按原样重放的同一请求——返回**第一次**调用创建的那个新 run id,数据库里只有一条 operation 和一条 retry run,task 不重复执行。去重以「同一意图复用同一 key」为前提,由客户端保证:dashboard 在 retry 意图的生命周期内持有并复用 key,请求 settle(成功或失败,无论响应是否到达)后清除,下次点击是新一轮意图、生成新 key;**不同客户端 / dashboard 副本各自生成各自的 key,是两个不同操作**——跨客户端去重需要服务端协调(如「每个源 run 只允许一次 retry」的强约束),不在本协议范围。**不同 key 是不同操作**,各自建新 run;operation 记录、run 创建、入队与 `work` 通知在同一个事务里提交。这个 key 空间与普通 trigger 的 `(task_id, idempotency_key)` 部分唯一索引**完全独立**——retry 幂等记录按源 run 定位,不是按 task。key 校验在 HTTP 边界:trim 后为空视为无 key(旧语义),超过 200 字符返回 400 `bad_request`(key 是客户端铸造的 token,200 字符远超合法长度,且远离 btree 索引条目上限,避免超长 key 把唯一索引 INSERT 打成裸 500)。
  - **不带该头的旧语义完全保留**:每次调用都是新的 retry,不写任何 operation 记录(已上线的无 body 路由因此无需迁移)。
  - **并发与交错**:retry 事务按 canonical lock order 先锁源 queue 行(通常不存在)再锁源 runs 行(FOR UPDATE),锁下重读状态——与 cancel/terminal 交错时,只有 failed/canceled 能产生 retry,其余状态稳定返回 409 `conflict`,源不存在 404 `not_found`。同 key 的并发请求被源 runs 行锁串行化:operation 行的 FK 校验会对源 runs 行持 KEY SHARE,与 FOR UPDATE 冲突,因此第二个写入者在锁上排队,等第一个提交后走「已提交记录回读」分支返回同一 run id。唯一索引冲突(23505)不是主竞态路径,只在写入绕过 FK(如 `session_replication_role=replica` 的测试写入)或未来 schema 变化时才可能触发,属防御纵深:冲突方整个事务回滚(刚建的新 run 不会成为孤儿),再回读已提交的 `retry_run_id` 返回;回读不到时抛稳定 409 `conflict`(附原始错误信息),不把裸 pg 错误漏给客户端。operation 行**没有独立 TTL**:行数以「每个源 run 的显式 retry 意图数」为界,随源 run 或 retry run 被 prune 时 CASCADE 删除——两端任何一端没了,该记录的回放答案就是死 id / 必然 404——即生命周期完全由 runs 的保留策略(kernel prune)决定,没有单独的清理循环。
  ...
  - **`idempotency_key` 故意不跟着走**:复用它会撞上源 run 自己的部分唯一索引,`/retry` 会把「被重试的那个 run 的 id」原样还回来。`env` 跟着走,`attempt`/`recoveries` 从头计。operation key 同理也不进 runs 表——它是「这次 retry 操作」的身份,不是新 run 的触发幂等键。
