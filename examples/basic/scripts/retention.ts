/* =============================================================================
   ...
   `ON DELETE CASCADE` the initial baseline ships on the run foreign keys,
   which is the entire reason pruning a run is one DELETE instead of four.
   This scenario proves it on a real database:

     1. a plain `DELETE FROM better_trigger.runs` takes that run's run_steps
        and logs with it, and leaves every other run's rows alone (the cascade
        exists, and is scoped to the run);
     2. the real CLI: `better-trigger-worker prune --older-than <w> --dry-run`
...
async function seedRun(
  ...
  await s.pool.query(
    `INSERT INTO better_trigger.run_steps (run_id, seq, kind, status) VALUES ($1, 1, 'step', 'completed')`,
    [id],
  );
  await s.pool.query(
    `INSERT INTO better_trigger.logs (run_id, level, message) VALUES ($1, 'info', 'a'), ($1, 'info', 'b')`,
    [id],
  );
}
...
async function main(s: Scenario): Promise<void> {
  ...
  await s.check('DELETE FROM better_trigger.runs cascades to run_steps and logs', async () => {
    await s.pool.query(`DELETE FROM better_trigger.runs WHERE id = 'run_cascade'`);
    s.assertEqual(
      await count(s, `SELECT count(*) FROM better_trigger.run_steps WHERE run_id = 'run_cascade'`),
      0,
      'run_steps of the deleted run',
    );
    s.assertEqual(
      await count(s, `SELECT count(*) FROM better_trigger.logs WHERE run_id = 'run_cascade'`),
      0,
      'logs of the deleted run',
    );
    // Scoped: the cascade must take that run's rows, not the table.
    s.assertEqual(
      await count(s, `SELECT count(*) FROM better_trigger.logs WHERE run_id = 'run_keep'`),
      2,
  ...
  await s.pool.query(
    `UPDATE better_trigger.runs SET created_at = now() - interval '90 days',
                     updated_at = now() - interval '90 days'
      WHERE id = 'run_stuck'`,
  );
  await s.pool.query(
    `INSERT INTO better_trigger.workers (id, code_version, runtime, tasks, concurrency, started_at,
  ...
  const runsBefore = await count(s, `SELECT count(*) FROM better_trigger.runs`);
  const logsBefore = await count(s, `SELECT count(*) FROM better_trigger.logs`);
  ...
  await s.check('prune --dry-run reports and deletes nothing', async () => {
    ...
    s.assertEqual(await count(s, `SELECT count(*) FROM better_trigger.runs`), runsBefore, 'runs after dry run');
    s.assertEqual(await count(s, `SELECT count(*) FROM better_trigger.logs`), logsBefore, 'logs after dry run');
    s.assertEqual(await count(s, `SELECT count(*) FROM better_trigger.workers`), 2, 'workers after dry run');
  });
  ...
  await s.check('prune deletes the terminal history past the window', async () => {
    ...
    const left = await s.pool.query<{ id: string }>(`SELECT id FROM better_trigger.runs ORDER BY id`);
    s.assertEqual(
    ...
    s.assertEqual(await count(s, `SELECT count(*) FROM better_trigger.logs`), 4, 'surviving logs');
    s.assertEqual(await count(s, `SELECT count(*) FROM better_trigger.run_steps`), 2, 'surviving steps');
    // Offline and past the window goes; the live one stays whatever its age.
    const workers = await s.pool.query<{ id: string }>(`SELECT id FROM better_trigger.workers`);
    s.assertEqual(
