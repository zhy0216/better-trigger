/* =============================================================================
   ...
   UPGRADE (0007 → latest):
     ...
     3. run the real `migrate()` — it must apply 0008..0013 and skip the
        already-journaled 0000..0007;
     4. the seeded rows survive with their values, the journal holds all 14
...
async function main(s: Scenario): Promise<void> {
  ...
  await s.check('migrate() upgrades 0007 → latest without touching the data', async () => {
    ...
    // The journal holds all 14 migrations — none pending, none re-run.
    s.assertEqual(
