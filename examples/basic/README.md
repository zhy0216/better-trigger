...
| `scripts/embedded.ts` | Live-Postgres acceptance for `createEmbeddedRuntime`: migration, registration, in-process trigger/result, durable steps and shutdown, on a `max:1` host pool with a host-first `search_path` beside same-name host tables. |
| `scripts/schema-isolation.ts` | Shared-database schema isolation through the daemon: host same-name tables in `public` (plus a `host_app` shadow) and a newer host `drizzle.__drizzle_migrations` survive boot, execution, dashboard/stats, the independent metrics probe pool and the prune CLI; only `better_trigger` is written and `SHOW search_path` never changes. |
...
| `scripts/retention.ts` | Data retention through the FK cascades: a manual `DELETE FROM better_trigger.runs` takes steps/logs with it, and the real `prune` CLI removes exactly the terminal runs past the window, their cascaded rows and offline worker rows. |
...
| `scripts/constraints.ts` | C5 database-level constraints: the five FKs (queue/waits.run_id cascade, `parent_run_id` + `child_run_id` SET NULL, schedules→tasks cascade) and the twelve CHECK constraints actually fire — a manual run delete leaves no orphan, a deleted child run's parent is failed by the orchestrator instead of stranded, an illegal status is refused with 23514, all of them ship in the initial baseline migration, and prune/cancel/retry keep every relation intact. |
...
| `scripts/migration.ts` | Baseline install beside a newer host Drizzle journal: the host's `drizzle.__drizzle_migrations` keeps its structure and sentinel rows, the dedicated `better_trigger.__drizzle_migrations` records the shipped baseline exactly, a re-run `migrate()` is a no-op, and two `max:1` pools racing the first install both succeed with one journal row. |
...
bun run --filter @better-trigger/example-basic e2e           # 20 checks · db _e2e · :4901
bun run --filter @better-trigger/example-basic embedded:acceptance # 6 checks · db _embedded · no daemon/port
bun run --filter @better-trigger/example-basic schema-isolation #  7 checks · db _schema_iso · daemon
bun run --filter @better-trigger/example-basic fencing       # 24 checks · db _fencing · no daemon
bun run --filter @better-trigger/example-basic replay-drift  # 30 checks · db _drift · :4903
bun run --filter @better-trigger/example-basic crash         # 14 checks · db _crash · :4902
bun run --filter @better-trigger/example-basic worker-lost   # 10 checks · db _worker_lost · :4904
bun run --filter @better-trigger/example-basic stats         #  4 checks · db _stats · :4906
bun run --filter @better-trigger/example-basic notify        # 10 checks · db _notify · :4907
bun run --filter @better-trigger/example-basic constraints    #  8 checks · db _constraints · no daemon
bun run --filter @better-trigger/example-basic health-pool    #  5 checks · db _health_pool · daemon (part 4)
bun run --filter @better-trigger/example-basic rolling-deploy # 11 checks · db _roll · :4911
bun run --filter @better-trigger/example-basic migration      #  4 checks · db _migrate · no daemon
